        ...
        <table className="w-full border-collapse rounded-lg shadow-lg overflow-hidden bg-white">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={12} className="text-center p-6">
                  <NoContent message="No submission found !" />
                </td>
              </tr>
            ) : (
              submissions?.data?.map((submission: TSubmission, index: number) => (
                <tr key={submission._id} className="hover:bg-gray-50 transition duration-200 text-sm border-b-[3px] border-b-blue-gray-100">
                  {/* Serial */}
                  <td className="sm:p-3 p-1 text-blue-gray-700 font-bold">
                    <RiNumbersFill className="inline mr-2 text-lg text-secondary" /> {index + 1}
                  </td>

                  {/* Contest Name */}
                  <td className="sm:p-2 p-1 text-blue-gray-500 font-poppins font-medium">
                    <div className="flex items-center gap-3">
                      <img
                        src={submission?.userId?.profileImage}
                        alt=""
                        className="h-12 w-12 sm:w-14 sm:h-14 rounded-full object-cover ring-2 ring-secondary"
                      />
                      <div>
                        <h4 className="font-medium sm:font-semibold text-SecondPrimary">{submission?.userId?.username}</h4>
                        <h4 className="sm:block hidden">{submission?.userId?.email}</h4>
                      </div>
                    </div>
                  </td>

                  {/* Deadline */}
                  <td className="sm:p-2 p-1 text-blue-gray-400">
                    <h4 className="font-medium sm:font-semibold text-SecondPrimary mb-1">{submission?.contestId?.title}</h4>
                    <div className="flex items-center">
                      <h6 className="font-medium sm:font-semibold text-blue-gray-900">Deadline:</h6>
                      <FaRegClock className="sm:inline mx-1 text-lg text-secondary hidden" />
...
export default ContestHolderSubmission;
